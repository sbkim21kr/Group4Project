...
# PLC 설정
PLC_IP = "192.168.3.39"  # todo ip 확인 필요
PLC_PORT = 5051          # todo 포토 확인 필요
PLC_TIMEOUT = 5.0
...
# =========================================================
# 로봇별 매핑 (포트 + M비트)
# 👉 여기에서 COM3/COM4/COM5/COM6 실제 포트에 맞게 수정!
# =========================================================

ROBOT_CONFIGS = [
    {
        "name": "R1",
        "port": "COM3",     # todo 포토 확인 필요
        "start_bit": "M100",
        "busy_bit":  "M101",
        "done_bit":  "M102",
...
def pulse_plc_bit(mc, address, on_time=0.2, off_time=0.2, tag=""):
    try:
        ...
        print(f"[PLC{tag}] Pulse {address} → ON {on_time}s")
        time.sleep(on_time)
        mc.batchwrite_bitunits(headdevice=address, values=[0])
        print(f"[PLC{tag}] Pulse {address} → OFF {off_time}s")
        time.sleep(off_time)
    except Exception as e:
...
def reset_dobot_state(api, name=""):
    ...
    try:
        ...
        # (선택) 홈 명령을 큐 없이 한 번 날려서
        # 내부 상태가 정상적으로 명령을 받는지 확인
        # temp 파라미터 필수!
        # 필요 없으면 주석 처리해도 됨
...
# =========================================================
# 로봇 1싸이클 처리: Connect → Run → Done → Disconnect
# =========================================================
...
def handle_robot_cycle(api, mc, cfg):
    ...
    if state != dType.DobotConnect.DobotConnect_NoError:
        ...
        # 실패했어도 PLC 쪽에서는 뭔가 응답이 필요하면 여기서 done 펄스 줄지 결정
    ...
    set_plc_bit(mc, "M403", 0, tag="m403 초기화")
    set_plc_bit(mc, "M404", 0, tag="m404 초기화")
    # 3) 시퀀스 실행
    try:
        if (isEmpty == False):
    ...
    # 5) 종료 전에 한 번 더 큐 정리(혹시라도 남은 거 방지)
    reset_dobot_state(api, name=name)
